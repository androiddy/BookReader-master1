       ...
       new AlertDialog.Builder(this)
                ...
                .setPositiveButton("确定", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        File d = null;
                        if(!books._id.contains("-dy-")){
                            d = new File(FileUtils.getChapterPath(books._id+"-dy-"+"123", 1));
                        }else{
                            d = new File(FileUtils.getChapterPath(books._id, 1));
                        }
                        if(d.exists()){
                          //  Toast.makeText(ScanLocalBookActivity.this,books.lastChapter+"---"+d.length(),0).show();
                            if(books.lastChapter.equals(FileUtils.formatFileSizeToString(d.length()))){
                                ToastUtils.showSingleToast("书籍已存在");
                            }else{
                                // 拷贝到缓存目录
                                FileUtils.fileChannelCopy(new File(books.path),
                                        d);
                                ToastUtils.showSingleToast("导入成功");
                            }
                        }else{
                            // 拷贝到缓存目录
                            FileUtils.fileChannelCopy(new File(books.path),
