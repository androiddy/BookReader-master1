...
function requestPage() {
	...
	$.getJSON(getUrl(), function(data){
		...
		content = formatContent(content, imgss)
//[INFO:CONSOLE(0)] "XMLHttpRequest cannot load http://www.gameqj.xyz:8080/Book/messages/findAllByMesId.do?id=2102. No 'Access-Control-Allow-Origin' header is present on the requested resource. Origin 'null' is therefore not allowed access.", source: file:///android_asset/themes/news/news.html (0)
...
// 预显示数据
function preDisplay() {
	var title = window.obj.getExtrastitle();
	var tag = window.obj.getExtrastag();
	$('.article-title').html(title)
	$('.source').html(tag)
}
...
// 组装请求的url
function getUrl() {
    var qStr = window.obj.getQueryStr();
	return qStr
}
